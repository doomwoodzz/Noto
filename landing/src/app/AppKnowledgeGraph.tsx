import { useEffect, useMemo, useRef, useState, type PointerEvent } from "react";
import {
  createInitialGraphBodies,
  dragGraphCluster,
  reconcileGraphBodies,
  stepGraphBodies,
  type GraphPoint,
} from "../noto/KnowledgeGraphPhysics";
import type { Graph as DemoGraph } from "../noto/types";
import type { GraphFilter, KnowledgeGraph } from "../noto-core";

interface Props {
  graph: KnowledgeGraph;
  activeFileId: string;
  onSelect: (id: string) => void;
  filter: GraphFilter;
  setFilter: (f: GraphFilter) => void;
}

const FILTERS: { id: GraphFilter; label: string }[] = [
  { id: "all", label: "All notes" },
  { id: "local", label: "Local" },
  { id: "lectureOnly", label: "Lectures" },
  { id: "orphans", label: "Orphans" },
];

const W = 880;
const H = 540;

export function AppKnowledgeGraph({ graph, activeFileId, onSelect, filter, setFilter }: Props) {
  const bounds = useMemo(() => ({ width: W, height: H, padding: 54 }), []);
  const svgRef = useRef<SVGSVGElement | null>(null);
  const dragRef = useRef<{ id: string; last: GraphPoint; moved: boolean } | null>(null);
  const grabbedIdRef = useRef<string | null>(null);
  const [grabbedId, setGrabbedId] = useState<string | null>(null);

  // The physics engine works on the demo graph shape (id/title/degree).
  const physicsGraph: DemoGraph = useMemo(
    () => ({
      nodes: graph.nodes.map((n) => ({ id: n.id, title: n.title, degree: n.degree })),
      edges: graph.edges.map((e) => ({ source: e.source, target: e.target })),
    }),
    [graph],
  );

  const [bodies, setBodies] = useState(() =>
    createInitialGraphBodies(physicsGraph, activeFileId, bounds),
  );

  useEffect(() => {
    let frame = 0;
    let previous = performance.now();
    function animate(now: number) {
      const dt = Math.min((now - previous) / 1000, 0.05);
      previous = now;
      setBodies((current) => {
        const reconciled = reconcileGraphBodies(current, physicsGraph, activeFileId, bounds);
        return stepGraphBodies(reconciled, physicsGraph.edges, bounds, dt, grabbedIdRef.current);
      });
      frame = requestAnimationFrame(animate);
    }
    frame = requestAnimationFrame(animate);
    return () => cancelAnimationFrame(frame);
  }, [activeFileId, bounds, physicsGraph]);

  function nodeSize(degree: number) {
    return 18 + Math.min(degree, 8) * 4;
  }

  function graphPoint(event: PointerEvent<SVGGElement>): GraphPoint | null {
    const svg = svgRef.current;
    if (!svg) return null;
    const transform = svg.getScreenCTM();
    if (!transform) return null;
    const point = svg.createSVGPoint();
    point.x = event.clientX;
    point.y = event.clientY;
    return point.matrixTransform(transform.inverse());
  }

  function beginDrag(event: PointerEvent<SVGGElement>, id: string) {
    const point = graphPoint(event);
    if (!point) return;
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    dragRef.current = { id, last: point, moved: false };
    grabbedIdRef.current = id;
    setGrabbedId(id);
  }

  function moveDrag(event: PointerEvent<SVGGElement>) {
    const drag = dragRef.current;
    const point = graphPoint(event);
    if (!drag || !point) return;
    const delta = { x: point.x - drag.last.x, y: point.y - drag.last.y };
    if (Math.hypot(delta.x, delta.y) > 0.2) {
      drag.moved = true;
      drag.last = point;
      setBodies((current) => dragGraphCluster(current, physicsGraph.edges, drag.id, delta, bounds));
    }
  }

  function endDrag(event: PointerEvent<SVGGElement>, id: string) {
    const drag = dragRef.current;
    if (event.currentTarget.hasPointerCapture(event.pointerId)) {
      event.currentTarget.releasePointerCapture(event.pointerId);
    }
    dragRef.current = null;
    grabbedIdRef.current = null;
    setGrabbedId(null);
    if (!drag?.moved) onSelect(id);
  }

  return (
    <div className="noto-graph">
      <div className="noto-graph-head">
        <div>
          <div className="noto-graph-title">Knowledge Web</div>
          <div className="noto-graph-sub">Generated from Markdown wiki links and backlinks.</div>
        </div>
        <div className="noto-segmented">
          {FILTERS.map((f) => (
            <button
              key={f.id}
              className={"noto-segment" + (filter === f.id ? " is-active" : "")}
              onClick={() => setFilter(f.id)}
            >
              {f.label}
            </button>
          ))}
        </div>
      </div>

      {graph.nodes.length === 0 ? (
        <div className="noto-empty" style={{ padding: 48 }}>No notes match this filter.</div>
      ) : (
        <svg
          ref={svgRef}
          width="100%"
          viewBox={`0 0 ${W} ${H}`}
          className="noto-graph-svg"
          preserveAspectRatio="xMidYMid meet"
        >
          <g>
            {graph.edges.map((e, i) => {
              const s = bodies[e.source];
              const t = bodies[e.target];
              if (!s || !t) return null;
              return (
                <line
                  key={i}
                  x1={s.x}
                  y1={s.y}
                  x2={t.x}
                  y2={t.y}
                  style={{ stroke: "var(--graph-edge)" }}
                  strokeWidth={1.4}
                />
              );
            })}
          </g>
          <g>
            {graph.nodes.map((n) => {
              const p = bodies[n.id];
              if (!p) return null;
              const isActive = n.id === activeFileId;
              const isGrabbed = n.id === grabbedId;
              const r = nodeSize(n.degree);
              return (
                <g
                  key={n.id}
                  className="noto-graph-node"
                  onPointerDown={(event) => beginDrag(event, n.id)}
                  onPointerMove={moveDrag}
                  onPointerUp={(event) => endDrag(event, n.id)}
                  onPointerCancel={(event) => endDrag(event, n.id)}
                  style={{ cursor: isGrabbed ? "grabbing" : "grab" }}
                >
                  <circle
                    cx={p.x}
                    cy={p.y}
                    r={r}
                    style={{
                      fill: isActive ? "var(--graph-node-active)" : "var(--graph-node)",
                      stroke: isGrabbed ? "var(--graph-node-grab)" : "var(--graph-node-stroke)",
                    }}
                    strokeWidth={isGrabbed ? 1.6 : 1}
                  />
                  <text
                    x={p.x}
                    y={p.y + r + 16}
                    textAnchor="middle"
                    fontFamily="Inter, system-ui"
                    fontSize="12"
                    fontWeight={isActive ? 600 : 400}
                    style={{ fill: "var(--graph-label)" }}
                  >
                    {n.title}
                  </text>
                </g>
              );
            })}
          </g>
        </svg>
      )}
    </div>
  );
}
